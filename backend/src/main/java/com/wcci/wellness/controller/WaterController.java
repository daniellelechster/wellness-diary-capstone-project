...
public class WaterController {
    ...
    public ResponseEntity<Water> getWaterByDate(@PathVariable("date") String date) {
    ...
    @PostMapping("/{id}/add")
    public ResponseEntity<Water> addGlass(@PathVariable("id") Long id) {
        Water updated = waterService.addGlass(id);
        return ResponseEntity.status(HttpStatus.OK).body(updated);
    }

    @PostMapping("/{id}/remove")
    public ResponseEntity<Water> removeGlass(@PathVariable("id") Long id) {
