...
function Calendar({ entries = {}, selectedDate, onDateSelect }) {
  ...
  const calendarDays = Array.from(
    ...
    (_, i) => {
      ...
      return {
        day,
        entry,
        moodColor: entry?.mood ? getMoodColor(entry.mood) : "#fff",
      };
    }
  );
  ...
  return (
    <div className="calendar-page">
      <h2>Your Calendar</h2>
      <img src={calendarImg} alt="Calendar Img" className="calendar-image" />
      <p>Track your emotional journey over time</p>

      <div className="calendar-stats">
        <div>
          <h4>Average Mood</h4>
          <p>{avgMoodThisMonth.toFixed(1)}/9</p>
        </div>
        <div>
          <h4>Days Logged</h4>
          <p>{daysLogged + 1}</p>
      ...
      {selectedEntry && (
        <div className="calendar-entry-details">
          ...
          {selectedEntry.waterIntake && (
            <p>Water: {selectedEntry.waterIntake} glasses</p>
          )}
          {selectedEntry.sleepHours && (
            <p>Sleep: {selectedEntry.sleepHours} hours</p>
          )}
        </div>
      )}
